// components/disbursement-platform/BulkApproveDialog.tsx
"use client";
...
interface BulkApproveDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (disbursementIds: string[], reason: string, pin: string) => Promise<void>;
  isLoading?: boolean;
  type: "IDR" | "USDT";
}
...
export const BulkApproveDialog: React.FC<BulkApproveDialogProps> = ({
  ...
  const fetchDisbursements = async () => {
    try {
      ...
      // Fetch disbursements with PROCESSING status only
      const processingResponse =
        await disbursementPlatformService.getDisbursementPlatforms(params);

      // Use only processing data
      const processingData = Array.isArray(processingResponse.data)
        ? processingResponse.data
        : [];

      setDisbursements(processingData);
    } catch (error) {
  ...
  const handlePinConfirm = async (pin: string) => {
    await onConfirm(selectedIds, reason.trim(), pin);
    setShowPinDialog(false);
  };

  const filteredDisbursements = disbursements.filter((d) => {
    const matchesSearch =
      d.platform_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const processingCount = disbursements.length;
  ...
  return (
    <>
        ...
        <DialogContent className="bg-white/95 backdrop-blur-xl border border-gray-200/50 shadow-2xl max-w-2xl max-h-[85vh] flex flex-col">
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div className="flex-1">
                ...
                <DialogDescription className="text-sm text-gray-600 mt-1">
                  Select disbursements to approve.{" "}
                  <span className="text-green-600 font-semibold">
                    This action will approve multiple disbursements at once.
                  </span>
                </DialogDescription>
              </div>
            </div>
          ...
          <div className="space-y-4 overflow-y-auto flex-1 min-h-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="relative md:col-span-2">
                ...
                <Input
                  ...
                  placeholder="Search by platform name, ID, or account number..."
            ...
            <div className="flex items-center gap-4 p-3 bg-gray-50/80 backdrop-blur-sm rounded-lg border border-gray-200">
              <span className="text-sm font-semibold text-gray-700">
                Showing Processing Disbursements: {processingCount}
              </span>
            </div>
            ...
            <div className="space-y-2">
              ...
              >
                Approval Reason <span className="text-green-500">*</span>
              </Label>
              <Textarea
                id="reason"
                value={reason}
                onChange={(e) => setReason(e.target.value)}
                placeholder="Enter reason for bulk approval..."
          ...
          <DialogFooter className="gap-2 mt-2 shrink-0">
            ...
            <Button
              ...
              className="bg-green-500 hover:bg-green-600 text-white font-medium shadow-lg disabled:opacity-50"
            >
              {isLoading ? (
      ...
      <PinInputDialog
        ...
        description={`Enter your PIN to approve ${selectedIds.length} disbursements`}
        isLoading={isLoading}
        actionType="confirm"
