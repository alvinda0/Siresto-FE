// app/disbursement-merchant-usdt/page.tsx
"use client";
...
import { DisbursementMerchantFilterUsdt } from "@/components/disbursement-merchant/DisbursementMerchantFilterUsdt";
...
const ListDisbursementMerchantUsdtPage = () => {
  usePageTitle("Disbursement Merchant USDT");
  ...
  const [selectedDisbursement, setSelectedDisbursement] = useState<DisbursementMerchant | null>(null);
  ...
  const [showBulkProcessingDialog, setShowBulkProcessingDialog] = useState(false);
  ...
  const [filters, setFilters] = useState<DisbursementMerchantQueryParams>({
    page: 1,
    limit: 10,
    type: "USDT", // Perbedaan utama ada di sini
  });

  const fetchDisbursements = useCallback(async () => {
    try {
      setLoading(true);
      const response = await disbursementMerchantService.getDisbursementMerchants(filters);
  ...
  const handleApplyFilters = (newFilters: DisbursementMerchantQueryParams) => {
    ...
    const isClearOperation = Object.keys(newFilters).length === 3 && 
                            newFilters.page === 1 && 
                            newFilters.limit === 10 && 
                            newFilters.type === "USDT";
    
    if (isClearOperation) {
      setFilters({ page: 1, limit: 10, type: "USDT" });
    } else {
      setFilters((prev) => ({ ...prev, ...newFilters, page: 1, type: "USDT" }));
    }
  };
  ...
  const handleBulkRejectConfirm = async (
    ...
    try {
      setBulkActionLoading(true);
      const response = await disbursementMerchantService.bulkRejectDisbursements({
        disbursement_ids: disbursementIds,
        reason,
      });
  ...
  const handleBulkProcessingConfirm = async (disbursementIds: string[]) => {
    try {
      setBulkActionLoading(true);
      const response = await disbursementMerchantService.bulkProcessingDisbursements({
        disbursement_ids: disbursementIds,
      });
        ...
        cell: (row) => (
          <div>
            ...
            <div className="text-xs text-gray-500">
              {row.bank_name !== "-" ? row.bank_name : "USDT Wallet"}
            </div>
          </div>
        ),
      },
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  router.push(
                    `/disbursement/usdt/detail/${row.disbursement_id}`
                  );
                }}
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-full">
      ...
      <DisbursementMerchantFilterUsdt 
      ...
      <div className="w-full">
        <CustomDataTable
          title="Disbursement Merchant USDT"
          description="List of all USDT disbursements to merchants"
...
export default withRoleProtection(ListDisbursementMerchantUsdtPage, [
  "Superuser",
  "Supervisor",
  "StaffFinance",
]);