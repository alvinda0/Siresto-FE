...
import { OrderMethod } from "@/types/order";
...
const EditOrderPage = () => {
  ...
  useEffect(() => {
    if (existingOrder && productResponse?.data) {
      setCustomerName(existingOrder.customer_name || "");
      setTableNumber(existingOrder.table_number || "");
      setOrderMethod(existingOrder.order_method);
      
      // Map order items to cart
      const cartItems: CartItem[] = existingOrder.order_items
        .map(item => {
          const product = productResponse.data.find(p => p.id === item.product_id) || item.product;
  ...
  const updateOrderMutation = useMutation({
    mutationFn: (payload: UpdateOrderPayload) => orderService.updateOrder(orderId, payload),
    onSuccess: () => {
    ...
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || "Gagal mengupdate order");
    },
  });
          ...
          <Card className="flex-1 flex flex-col">
            ...
            <CardContent className="flex-1 flex flex-col">
              ...
              <div className="flex gap-2 mt-4">
                ...
                >
                  {updateOrderMutation.isPending ? "PROSES..." : "UPDATE PESANAN"}
                </Button>
              </div>
            </CardContent>
          </Card>
